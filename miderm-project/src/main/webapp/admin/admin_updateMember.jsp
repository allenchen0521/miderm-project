...
<head>
<meta charset="UTF-8">
<title>admin_updateMember</title>
</head>
<style>
* {
	font-family: 微軟正黑體;
				...
				<fieldset>
					...
					<div class="form-group">
						<label for="mem_name">會員名稱:</label> <input type="text"
							...
							placeholder="請輸入會員名稱"> <span class="error">${errorMsg.errPassword}</span>
					</div>
					<div class="form-group">
						<label for="mem_level">會員等級:</label> <input type="text"
							class="form-control" id="mem_level" name="mem_level" value="${member.mem_level}"
							placeholder="請輸入會員等級"> <span class="error">${errorMsg.errPassword}</span>
					</div>
				</fieldset>
				<button id="modiBtn" class="btn btn-info col-md-12">確認修改</button>
			</div>
		</div>
	</div>
<script>
...
$("#modiBtn").click(function(){
	$.ajax({
		url: "http://localhost:8080/" + requestUrl + "/admin/UpdateMemberProcessServlet",
		type: "post",
		data: {
			mem_id: $("#mem_id").val(),
			mem_name: $("#mem_name").val(),
			mem_level: $("#mem_level").val()
		}
	});
